...
export class CustomerCourtesyCardService extends TableService<CourtesyCard> implements OnDestroy {
  
  API_URL = `${environment.apiUrl}/customercourtesycard?lot_no=`;
  ...
  find(tableState: ITableState): Observable<TableResponseModel<CourtesyCard>> {
    return this.http.get<CourtesyCard[]>(this.API_URL+this.LOTNO).pipe(
